...
*   **Apache Maven:** This project uses Maven for building and dependency management. Ensure you have Maven installed. You can typically download it from the [Apache Maven website](https://maven.apache.org/download.cgi).
*   **Gson Library:** This server uses the Gson library for JSON parsing and serialization. Maven will automatically download this dependency.
...
*   `pom.xml`: Maven Project Object Model file, defines project configuration, dependencies, and build instructions.
...
This project uses Apache Maven. You can compile and run the server with a single command from the project's root directory:
```bash
mvn compile exec:java
```
This command will compile the source code and then execute the main class `io.modelcontextprotocol.server.Main`. The server will start and listen for JSON-RPC messages on standard input (STDIO).
